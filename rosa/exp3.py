...
def main():
    ...
    for epoch in range(1, Config.NUM_EPOCHS + 1):
        ...
        history_baseline['val_acc'].append(acc_base)
        print(f"  - Baseline Model: Train Loss: {loss_base:.4f} | Val Acc: {acc_base:.2f}% | Time: {duration_base:.2f}s")

        start_time = time.time()
        loss_rosa = train(rosa_model, train_loader, optimizer_rosa, criterion)
        duration_rosa = time.time() - start_time
        acc_rosa = validate(rosa_model, val_loader)
        history_rosa['val_acc'].append(acc_rosa)
        print(f"  - ROSA Model    : Train Loss: {loss_rosa:.4f} | Val Acc: {acc_rosa:.2f}% | Time: {duration_rosa:.2f}s")

    print("\n--- 训练完成 ---")
    final_acc_base = history_baseline['val_acc'][-1]
    final_acc_rosa = history_rosa['val_acc'][-1]
    ...
    plt.plot(history_baseline['val_acc'], label='Baseline Transformer Accuracy', marker='o')
    plt.plot(history_rosa['val_acc'], label='ROSA-Augmented Transformer Accuracy', marker='x')
    plt.title('模型学习曲线对比 (验证集准确率)')
    plt.xlabel('Epoch')
    plt.ylabel('Validation Accuracy (%)')
    plt.legend()
    plt.grid(True)
    plt.xticks(range(Config.NUM_EPOCHS))
    plt.ylim(0, 101)

    print("\n--- 学习过程可视化 ---")
    print("在Notebook环境中，将直接显示图表...")
    plt.show()
